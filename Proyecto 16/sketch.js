...
function Monster(){
  var spawn = 200
  var monsterx = 0
  if(World.frameCount%spawn===0){
    monster=createSprite(550,200,20,20);
    monster.addAnimation("moving", monsterImage);
    monster.y=Math.round(random(50,350));
    monsterx=Math.round(random(0,600));
    monster.velocityX=-(8+(score/10));
    monster.velocityY=-(-8+(score/12));
    ...
    if(monsterx > 0 && monsterx < 20){
      monster.x = monsterx;
      monster.velocityX = monster.velocityX *-1;
     }

     if(monsterx > 560 && monsterx < 600){
      monster.x = monsterx;
     }
...
function fruits(){
  ...
  //Aumentar la velocidad de las frutas después de 4 puntos 
   ...
   //Agregar gravedad a las frutas
      fruit.velocityY= (2+(score/6));
    ...
    fruitpos=Math.round(random(0,600));

    //Codigo para que al aparecer en el lado derecho se mueva a la derecho
   if(fruitpos > 400 && fruit.y < 350){
    fruit.x = fruitpos;
